...
//@desc Create Resume
// @access Private
router.post('/resumeForm', 
// passport.authenticate("jwt", { session: false }),
[
    check('firstName').not().isEmpty().withMessage('First name field is required'),
    check('lastName').not().isEmpty().withMessage('Last name field is required'),
    check('address').not().isEmpty().withMessage('Address field is required'),
    check('address', 'Address must be between 4 and 40 charachters').trim().isLength({ min: 4, max: 40 }),
    check('city').not().isEmpty().withMessage('City name field is required'),
    check('state').not().isEmpty().withMessage('State field is required'),
    check('zipcode').not().isEmpty().withMessage('Zipcode field is required'),
    check('personalEmail').not().isEmpty().withMessage('Email field is required'),
    check('githubURL').not().isEmpty().withMessage('GitHub URL is required'),
    // check('githubURL').not().isURL('githubURL').withMessage('Invalid Github link'),
    check('twitterURL').not().isEmpty().withMessage('Twitter field is required'),
    // check('twitterURL').not().isURL('twitterURL').withMessage('Invalid Twitter link'),
    check('personalWebsiteURL').optional(),
    check('aboutMeStatement').not().isEmpty().withMessage('About me field is required'),
],
(req, res) => {
